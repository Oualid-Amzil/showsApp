...
const LogInScreen = ({ navigation }) => {
  ...
  const loginHandler = () => {
    if (emailState.valid && passwordState.valid) {
      dispatch(signIn(emailState.value, passwordState.value, navigation));
      emailDispatch({ type: "INPUT_CHANGE", value: "", valid: false });
      emailDispatch({ type: "INPUT_BLUR", value: false });
      passwordDispatch({ type: "INPUT_CHANGE", value: "", valid: false });
      passwordDispatch({ type: "INPUT_BLUR", value: false });
    } else {
            ...
            {isLoading ? (
              ...
              <Pressable style={styles.button} onPress={loginHandler}>
                <Ionicons
                  ...
                  color="black"
                />
                <Text style={{ fontSize: 19, marginLeft: 5 }}>LogIn</Text>
              </Pressable>
            )}
          </View>
        </ScrollView>
